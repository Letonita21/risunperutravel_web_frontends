  ...
  return (
    <>
      ...
      <section className="pt-5 pb-10 px-4 md:px-8 lg:px-16 justify-items-center">
        ...
        />
        <button className="flex gap-2 mb-3 px-10 py-2 bg-amber-700 text-white hover:bg-yellow-500 rounded-lg font-semibold text-sm shadow">
          ...
          >
            <div className="flex gap-5">
              {[...testimonials, ...testimonials].map((item, index) => (
                <div
                  key={index}
                  className="block w-80 md:w-96 bg-white rounded-lg p-3 border-4 h-50 border-l-blue-800 border-t-red-800 border-r-amber-400 border-b-green-700  drop-shadow-xl drop-shadow-gray-300"
                >
                  ...
                  <div className="flex space-x-1  my-2 text-gray-500">
                    {[...Array(item.calificacion)].map((_, index) => (
                      <StarIcon key={index} color={"#FFC014"} />
                    ))}
                    <p>{item.date}</p>
                  </div>
                  <p className="text-sm line-clamp-5 text-gray-800 scroll-auto ">
                    {item.text}
                  </p>
                </div>
              ))}
