          ...
          {tours.map((tour, index) => (
            ...
            >
              ...
              >
                ...
                />
                <div className="absolute top-5 left-2 bg-[#A30923] text-white font-semibold px-5 py-0.5 rounded-md text-md shadow-md shadow-gray-800">
                  {dict.desde}:{" "}
                  <p className="text-2xl font-bold drop-shadow-md -mt-2 drop-shadow-gray-600">
                    ${tour.precio_enganche}
                  </p>
                </div>
                ...
                <div
                  className={`absolute left-0 w-full p-4 text-white z-10 transition-all duration-500 ${
                    hovered ? "bottom-20" : "bottom-0"
                  }`}
                >
                  <p className="flex gap-1 text-lg items-center font-semibold opacity-90 drop-shadow-sm drop-shadow-gray-800">
                    {tour.dificultad} <ShoeIcon /> / {tour.picos}{" "}
                    <MountainIcon />
                  </p>
                  <p className="flex gap-1 text-lg items-center font-semibold opacity-90 drop-shadow-sm drop-shadow-gray-800">
                    <ClockIcon /> {tour.duracion}{" "}
                    {tour.duracion > "1" ? dict.dias : dict.dia}
                  </p>
                  <p className="flex gap-1 text-lg items-center font-semibold opacity-90 drop-shadow-sm drop-shadow-gray-800">
                    <LugarIcon /> {tour.departamento}
                  </p>
                  <h2 className="text-xl font-bold drop-shadow-sm drop-shadow-gray-800">
                    {tour.nombre}
                  </h2>
                </div>
                {/* Botón */}
                <div
                  className={`absolute bottom-0 left-0 w-full p-5 transition-all duration-500 ${
                    hovered
